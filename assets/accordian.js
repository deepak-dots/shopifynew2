...
(function($) {
	var settings;
	$.fn.accordian = function(actionOrSettings, parameter) {
		...
		if (actionOrSettings == "open") {
			...
			$(settings.headline).accordian('close');
		} else if (actionOrSettings == "forceCloseAll") {
			// forceCloseAll ignores collapsible setting
			$(settings.headline).accordian('forceClose');
		}
		...
		$(settings.headline, this).click(function() {
			if ($(this).hasClass('active')) {
				...
				$(this).accordian('open', settings.scroll);
			}
		});
	};
}(jQuery));
